...
Apart from using the web front-end in the client-direct, it also offers a command-line tool for starting simulations. In order to call this tool, it needs a setup with Python, the same installed components as the web front-end and access to the database. The most conveniant way to acheive this is to start a docker component specifically for the purpose of starting simulations in command line.|br|
The exmaple below starts a docker component and shows the various command line options available. **Please note** that any path to PK data files is local to the docker component.|br|
``
sudo docker run -it --rm --net ap_nimbus_network -v ap_nimbus_file_upload:/opt/django/media --env-file env cardiacmodelling/ap-nimbus-client-direct:0.0.8 python manage.py start_simulation -h
