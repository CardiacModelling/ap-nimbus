...
export class ResultsComponent implements DoCheck, OnChanges {
  ...
  showYAxisLabel = true;
  //legendTitle = 'Conc (µM)';
  legendTitle = 'Simulation';
  //xAxisLabel = 'Time (ms)';
  xAxisLabel = 'Concentration (µM)';
  ...
  // Amount to expand y axis by if beyond default values.
  yAxisRescale: number = 10;
      ...
      newArrivalsKeys.forEach((simulationId) => {
        ...
        if (typeof simulationResults !== 'undefined') {
          ...
          this.allSimulationResults[simulationId] = {
            'timer' : timer(0, 500).subscribe((val) => { this.sub(simulationId, metaData) }),
            'completed' : false,
            'voltage_results' : []
          };
        }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (typeof simulationsOutputChanges !== 'undefined') {
      ...
      if (experimentalOnly.length > 0) {
        ...
        this.chartData = experimentalOnly;
      } else {          
  ...
  private assignDefaultAxes(): void {
    ...
    // TODO: Needs to end on an integer value (for dynamic changes!)
  ...
  private processVoltageResults(apPredictOutput: ApPredictOutput, simulationId: string,
    ...
    if (apPredictOutput.hasWaitingMessage()) {
      ...
      let minMax = {
        x: { min: this.xScaleMax, max: this.xScaleMin },
        y: { min: this.yScaleMax, max: this.yScaleMin }
      };

      JSON.parse(voltageResults).forEach((perConcentrationData) => {
        let concentration = perConcentrationData['c'];
        if (concentration > 0.003) {
          let deltaAPD90 = perConcentrationData['da90'];
          if (!Number.isNaN(Number.parseFloat(deltaAPD90))) {

            let dataPointConc = this.log10(Number.parseFloat(concentration));
            let dataPointDeltaAPD90 = Number.parseFloat(deltaAPD90);

            if (dataPointConc < minMax.x.min) {
              minMax.x.min = dataPointConc;
            }
            if (dataPointConc > minMax.x.max) {
              minMax.x.max = dataPointConc;
            }
            if (dataPointDeltaAPD90 < minMax.y.min) {
              minMax.y.min = dataPointDeltaAPD90;
            }
            if (dataPointDeltaAPD90 > minMax.y.max) {
              minMax.y.max = dataPointDeltaAPD90;
            }

            seriesData.push({
              name: dataPointConc,
              value: dataPointDeltaAPD90
            });
          }
        }
      });
      ...
      let newSeriesCnt = seriesData.length;

      let found: boolean = false;
      this.chartData.forEach((eachChartData) => {
        let eachChartDataName = eachChartData['name'];
        let eachChartDataSeries = eachChartData['series'];

        if (eachChartDataName == metaData['title']) {
          found = true;
          if (eachChartDataSeries.length != newSeriesCnt) {
            eachChartData['series'] = seriesData;
          }
          return;
        }
      });
      ...
      //if (this.chartDataStore != voltageResults) {
      //  this.chartData = JSON.parse(voltageResults);
      //  this.chartDataStore = voltageResults;
      //}
    } else {
  ...
  private sub(simulationId: string, metaData: object): void {
  ...
  xAxisTickFormatting(val: number): number {
    let decimalPlaces = val >= 0 ? 0 : Math.round(Math.abs(val));
  ...
  /*
  private processVT(apPredictOutput: ApPredictOutput, simulationId: string): void {
    if (apPredictOutput.hasWaitingMessage()) {
      ...
      if (this.chartDataStore != voltageTraces) {
        this.chartData = JSON.parse(voltageTraces);
        this.chartDataStore = voltageTraces;
      }
    } else {
      // No data - looks serious!
      this.vtMessage = null;
