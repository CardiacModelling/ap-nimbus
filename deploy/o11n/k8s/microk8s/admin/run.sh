...
if [ "${cmd}" == "delete" ]; then
  ...
  microk8s.kubectl create secret generic ${secret} --namespace=${namespace} --from-file=username=/var/lib/jenkins/username.txt --from-file=username=/var/lib/jenkins/password.txt
fi
